...
public sealed partial class World
{
    ...
    public string? PlacementProblem(Cell cell, int rotated, BuildingKind kind = BuildingKind.Cottage) => rotated is <0 or >3 ? "Choose one of four building orientations." : kind==BuildingKind.HuntingLodge && !HuntingGrounds(cell).Any(h=>HabitatCapacity(h)>0) ? "Hunting lodge needs reachable wooded habitat within 8 tiles. Find wildlife on Three clearings; retain mature trees." : kind==BuildingKind.Quarry && !Map.StoneDeposits.Any(d=>d.Remaining>0 && (d.Cell.Point-cell.Point).LengthSquared()<=16 && Accessible(d.Access)) ? "Quarry needs a reachable, unexhausted stone outcrop within 4 tiles. Use Resource survey [U] to find stone." : kind == BuildingKind.FishingDock ? DockProblem(cell, rotated) : kind == BuildingKind.Bridge ? BridgeProblem(cell, rotated) :
