...
public sealed partial class World
{
    ...
    public int MealVarietyScore => Food.LastMealRequired == 0 ? 0 : Math.Min(20, (int)MathF.Round(20f *
        (Food.LastMealBerries + Food.LastMealVegetables + Food.LastMealBread - Math.Max(Food.LastMealBerries, Math.Max(Food.LastMealVegetables, Food.LastMealBread))) /
    ...
    public string LastMealSummary => Food.LastMealRequired == 0 ? "No meal served yet." :
        $"Last meal: {Food.LastMealBerries + Food.LastMealVegetables + Food.LastMealBread}/{Food.LastMealRequired} portions eaten\n{Food.LastMealBerries} berries · {Food.LastMealVegetables} vegetables · {Food.LastMealBread} bread\nVillage meal variety: +{MealVarietyScore}/20. More evenly shared food types give a larger benefit.";
    private void ValidateHappiness()
    {
        if (Food.LastMealRequired < 0 || Food.LastMealBerries < 0 || Food.LastMealVegetables < 0 || Food.LastMealBread < 0 ||
            Food.LastMealBerries + Food.LastMealVegetables + Food.LastMealBread > Food.LastMealRequired ||
            Food.LastMealChoices != new[] { Food.LastMealBerries, Food.LastMealVegetables, Food.LastMealBread }.Count(n => n > 0))
