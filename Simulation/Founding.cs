...
public sealed class FoundingProgress
{
    ...
    public bool GroupedFarmsteads { get; set; }
    public bool AcrossTheInlet { get; set; }
...
public sealed partial class World
{
    ...
    private void ValidateFounding()
    {
        ...
        if(f.TransformationHamlet && (f.StartingBuildings==null || f.StartingBuildings.Count!=(f.PlayerFounded?0:9) || f.StartingBuildings.Select(b=>b.Id).Distinct().Count()!=(f.PlayerFounded?0:9) || f.StartingBuildings.Any(b=>b.Id<1 || b.Id>=_nextSite || b.Rotation is <0 or >3 || !Enum.IsDefined(b.Kind) || !Map.Contains(b.Cell))))throw new InvalidOperationException("Invalid hamlet opening layout");
        if(f.AcrossTheInlet && (!f.CultivatedBank || f.GroupedFarmsteads))throw new InvalidOperationException("Invalid inlet comparison");
        if(f.GroupedFarmsteads && !f.CultivatedBank)throw new InvalidOperationException("Grouped farmsteads need the working-bank inventory");
