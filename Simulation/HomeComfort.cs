...
public sealed partial class World
{
    ...
    public string ComfortSummary(Cottage home)
    {
        ...
        if(Founding?.RiverFarmstead==true && !home.ImprovementRequested)
            return $"{PotentialHomeYardPlaces(home).Length}/2 outdoor places usable · {YardSideName(home.YardSide)}. "+(PotentialHomeYardPlaces(home).Length==0?"Clear the selected yard side to restore outdoor use. ":"")+(home.Improved?"Furnished yard · residents mend and take nearby meals outside at home. Ordinary meals use the shorter eligible trip to home or shared ground. Keep the selected yard places clear. Rest also lasts longer; no extra beds.":$"Furnish this occupied home for {ComfortCost(home)} planks. Residents use the yard during quiet work time and bring nearby meals home. {(PublicPlace!=null?"Shared workers deliver and install the planks":"Requires carpenter and planks")}; keep the selected yard places clear.");
