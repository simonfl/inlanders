param([switch]$SmokeTest, [switch]$AudioSmokeTest, [switch]$HudSmokeTest)
...
if ($SmokeTest -or $AudioSmokeTest -or $HudSmokeTest) {
    $engine = $engine.Replace('_win64.exe', '_win64_console.exe')
    $testArgument = if ($HudSmokeTest) { '--hud-smoke-test' } elseif ($AudioSmokeTest) { '--audio-smoke-test' } else { '--smoke-test' }
