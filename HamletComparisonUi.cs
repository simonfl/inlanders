...
public partial class Game
{
    ...
    private int _hamletComparisonHash;
    private int HamletLayoutHash(){var hash=new System.HashCode();foreach(var site in _world.Cottages){hash.Add(site.Id);hash.Add(site.Cell);hash.Add(site.Rotation);hash.Add(site.Kind);}return hash.ToHashCode();}
    private void MakeHamletComparisonUi()
    {
        ...
        var legend=Text("OPENING / NOW\nGold · opening footprints\nBlue · moved or added places\nOnly changed footprints are outlined. Daily life continues.",14,true);legend.CustomMinimumSize=new(280,0);column.AddChild(legend);
        _hamletCompareDone=Button("Hide comparison [Esc]",()=>_courtShowBefore=false);column.AddChild(_hamletCompareDone);_hamletComparePanel.Hide();
    }
    private void RenderHamletComparison()
    {
        ...
        if(show && _hamletComparisonHash!=HamletLayoutHash())MakeCourtStartingLayout();
