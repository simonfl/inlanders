...
Design decision for a prototype, not shipped behavior. **F07c1 accounting experiment is complete:** see [decisions, measurements and limits](MEAL_EXPERIMENT_F07C1.md). F07c2 playable integration is next. The aim is an inhabited food destination and a useful choice between a compact village and supplied outlying neighborhoods. Do not add a pantry that is merely another global food counter.
...
- A collected portion may finish up to 30 seconds late; afterward return it physically. A request with no collected portion expires at its deadline and releases its claim. The next request follows the original cadence, recording skipped slots rather than accumulating meal debt. F07c1 tests this bounded exception; the UI must explain it.
- Count portions when eaten. F07c1 separates nourishment from reliable service: missed deadlines clear the resident's fed state, actual eating restores it immediately, and late consumption never rewrites a missed outcome. Hunger is the unfed resident fraction; retain the maximum half-speed penalty initially. For campaign integration use a trailing 180-second service window, at least two closed requests per resident and no missed/skipped service, alongside actual variety and fresh delivery proof. See the experiment review for startup/pending-window rules. Revalidate campaign timings in F07c2.
...
F07c2 working-fixture decision: haulers also return unreserved local surplus above the target to central storage. A target of zero drains a pantry through actual trips; direct producer deposits and resident meals continue. This allows locally deposited bread to reach the central supper without demolishing its pantry. Transfers in either direction are not fresh production. Keep local-to-local redistribution deferred. The compact comparison does not establish a universal pantry benefit: setup and hauling can cost more than the resident travel they save.
