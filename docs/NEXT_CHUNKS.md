...
Relocation and the campaign diagnostic are complete; F07d local stone storage review can proceed independently while F10b2/F17b await listening feedback. Musical composition still follows the sound review.
...
- **Village arrangement:** gates with explicit walking rules, richer planting and area tools; Creative terraces are delivered; normal-play shaping costs and broader sculpting remain separate.
...
## F17b1 delivered — music sequencing and quiet intervals

[Verified transport](MUSIC_SEQUENCING_F17B1.md): natural completion, timed silence, menu/load preservation, live mixer and settings checks. Existing piece only; new themes and subjective review remain open.
...
## F16e delivered — Creative central resource setup

[Controls and accounting](CREATIVE_STOCK_F16E.md) deliver ten resources, protected reservations, explicit Apply, separate ledgers and save 35. Simulation and narrow/wide UI checks pass. The original brief below is retained.
...
## F12h delivered — Creative terraces

**F12h1 foundation delivered:** [Reviewed policy and geometry evidence](TERRAIN_SHAPING_F12H1.md). Three distinct usable terraces pass; their nine-cell selections affect 25–45 cells. [F12h2 authoritative preview/apply and terrain-only undo](TERRAIN_SHAPING_F12H2.md) is implemented. [F12h3 controls and presentation review](TERRAIN_SHAPING_F12H3.md) are delivered: drag selection, Before/After, full-border feedback, Apply/Undo and rendered picking checks. The combined feature counts once as playable checkpoint 4. The original brief follows.
...
## F09e delivered — always-open fence gateways

**F09e1 design delivered:** [Decision and acceptance checks](FENCE_GATEWAY_F09E1.md). Implement the always-open, path-compatible gateway with four saved facings and connections only at its side posts. Preserve service workspaces, actual walking cargo and the free decoration economy. Compare its readability with an empty gap before claiming the player outcome. [F09e2 implementation and presentation review](FENCE_GATEWAY_F09E2.md) now delivers the gateway. The combined feature is playable checkpoint 5. The original brief follows.
...
If a gate adds a useful readable entrance, implement the smallest coherent version with four-way visuals and ordinary placement feedback, preserve existing routes and decorations, and verify normal/Creative costs, current saves and narrow/wide controls. If it only duplicates an unmarked gap without improving readability, revise or cut the candidate. No new campaign requirement or animal-enclosure economy. After the next playable outcome, stop implementation for the checkpoint-5 independent four-role review and use its findings to select the next five chunks.

## 3. Checkpoint-five periodic review — due now

Before further implementation, run the four independent read-only roles on the F09e2 commit/build: game designer, UX/onboarding, playtest and game development lead. Apply the accepted cadence, use concrete evidence and explicitly record any unavailable UI or human evidence. Consolidate overlapping findings into fix now, investigate next, defer or cut; amend the roadmap and select the next five chunks. The gateway presentation review is recorded separately and does not satisfy this periodic review.
