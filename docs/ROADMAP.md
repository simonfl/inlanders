...
**F23a is implemented and awaiting visual feedback.** Review the [matched before/after scene](ART_REVIEW_F23A.md) before expanding the style. **Across the river (F11b1 / F18b1) is now playable; pacing acceptance remains open.** Two scripted approaches and recovery checks pass, but they do not establish human enjoyment or duration. **Next chunk: F23c — measured rendering costs**, prompted by roughly 49 ms frames in the 16-resident river scene. Then F25a deepens resident routines using existing buildings. F23b follows visual acceptance or another focused art iteration. See [river findings](ACROSS_THE_RIVER.md) and the [resident-needs and campaign plan](CAMPAIGN_SYSTEMS.md).
...
| 1 | F23c | **Rendering cost at village scale.** Profile the 16-resident river scene, a decorated scene and a prolonged paused placement preview. Investigate repeated meshes/materials, yard stock and draw calls; optimize measured costs while retaining architectural depth. | Record comparable before/after frame distributions and scene counts on this machine. Preserve visible buildings, cargo, selection and placement behavior. Choose the implementation after profiling; no broad engine rewrite. |
