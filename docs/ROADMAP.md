...
**Next: a substantial worked-land and place-composition slice**, selected by [whole-game review76](REVIEW_CHECKPOINT_76.md). Across the inlet now supports keeping the village, crossing the channel or redistributing a home and field through ordinary rules. It is a useful experiment, not proof of an enjoyable loop. Aggregate walking improvements do not establish efficiency or a wanted change.

Test F40b/c on the existing inlet: actual cultivated ground, domestic clearings, worn approaches, woodland margins and river transitions should make an inhabited farmstead coherent at ordinary zoom. Keep the present presentation as control, the same people/capacity/rules and all three responses. Couple a restrained place-focused interface where necessary; do not split it into more isolated UI checkpoints or add household policies/venues by default.
...
The [active queue](NEXT_CHUNKS.md) and [decision evidence](INLET_COMPARISON_76.md) define the experiment and rejection criteria. Count **80**. Five independent roles completed review76 (three fresh, two reused contexts); no human play, independent discovery, listening or native performance verdict. Review was brought forward after one coherent outcome; periodic80 remains due. User-authorized run now continues through90 with full reviews80/85/90.77 adds dynamic worked-land treatment and secondary comparison entry; visual acceptance remains provisional. 78 now exposes actual selected-place trips, framing/following and arrival. 79 reshapes actual banks/woodlots, retaining inventory and viable alternatives;80 lets actual eligible trip cost choose home/shared meals. Full review80 now chooses the next batch; F40 remains partial. Current-save replacement denial remains unresolved; precise partial-save feedback is a separate correction.
