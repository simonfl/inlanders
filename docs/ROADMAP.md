...
**F23a is implemented and awaiting visual feedback.** Review the [matched before/after scene](ART_REVIEW_F23A.md) before expanding the style. **Across the river (F11b1 / F18b1) is now playable; pacing acceptance remains open.** Two scripted approaches and recovery checks pass, but they do not establish human enjoyment or duration. **F23c now batches fixed building and decoration geometry**, reducing measured frame time without removing detail; see the [rendering review](RENDERING_F23C.md). **F11b2 now gives Life by the lake a constrained home shore, limited natural food and consequential service placement.** Deliberate routes take 10/18 scripted simulation minutes; adding a nearby square recovers a rough layout in 16 instead of 25. Human pacing remains unverified. **F02b now preserves chosen trees and maintains small groves with existing loggers.** See the [woodland review](MANAGED_WOODLAND_F02B.md). **F07b now shows actual supply routes and supports local plank storage.** A nearby plank pile saves travel in the remote construction fixture; compact/awkward piles do not repay setup. Food service remains a later F07c design. **F03b1 now gives logging progress-driven strikes and a brief tree fall. F03b2 handoffs and hammer work have also shipped. Next: F03b3 — field tending and harvest**, then F04b home/social presentation; see the [work-animation plan](WORK_ANIMATION_F03B.md); see the [logistics findings](LOGISTICS_F07B.md). See [lake findings](LAKE_DIFFICULTY_F11B2.md). F23b follows visual acceptance or another focused art iteration. See [river findings](ACROSS_THE_RIVER.md) and the [resident-needs and campaign plan](CAMPAIGN_SYSTEMS.md).
...
| 1 | F03b3 | **Field tending and harvest.** Logging, delivery handoffs and hammer work have shipped. Next distinguish sowing, soil work, grain harvest and vegetable picking with poses that reach their targets. Reuse F22 crop stages. | Readable actions at the ordinary camera, tied to crop/work state and actual cargo. Pause, interruption and reload stay correct; no changed production rate or phantom harvest. |
| 2 | F04b | **Inhabited homes and gathering places.** Present actual F25 rest visits and square participation with a small set of social moments and recognizable residents. Need rules remain in F25. | Residents face their destination or one another plausibly; breaks feel inhabited without constant gestures or new social meters. |
...
**Next new campaign-system recommendation: F26b + F25c, quarry and gathering hall.** After current woodland/logistics work and campaign feedback, prototype a substantial stone/timber venue against distributed squares using actual recreation visits. A nearby small outcrop versus a distant larger deposit creates the map decision; a visibly used hall supplies the payoff. Start without a mandatory stonecutter or entertainer. F26c then tests mature woodland habitat against clearing and hunting pressure, with separate, recoverable consequences. See the [concrete scenario briefs and broader building candidates](CAMPAIGN_SYSTEMS.md#next-design-decisions-make-each-addition-earn-its-place).
