...
| 1 | F11b2 / F18b2 | **Campaign decisions and difficulty.** Rework the lake or its follow-on challenge around constrained production sites, competing labor and an explained expansion decision. Use existing fishing, food, homes and recreation before adding another producer. Review river pacing alongside it. | Several consequential choices after the opening, two viable approaches and a recoverable inefficient plan. Current lake routes take 7/9 simulation minutes and accumulate large food surpluses; do not call this mastery or extend it with quotas, slower work or longer streaks. |
...
**F11b2 / F18b2 — next difficulty experiment:** the lake introduction now has two successful routes, but both accumulate large reserves and finish in 7/9 scripted minutes. Before another new resource, author and compare a fishing-led plan and a cultivation-led plan under meaningful constraints on productive sites, available workers and expansion. Start by testing whether generous berry supply and unconstrained dry land remove the need to choose. Expose the consequences of a premature expansion or overbuilt fishery, and prove a recovery using different staffing/layout. Keep the current twelve-resident scale unless a measured reason calls for growth; neither bigger population nor a longer meal streak is a substitute for decisions. Review whether to revise the lake itself or retain it as an optional introduction followed by a distinct challenge. Final geography and thresholds remain TBD.
...
**First chunk implemented:** [Across the river](ACROSS_THE_RIVER.md) supports opening → crossing → prepared growth → supported neighborhood, including a cottage/garden and bread/lodge approach. The east-bank square must actually serve residents. F25a now assigns individual homes and rest visits; river goals still use east-bank bed capacity. Next campaign work is F11b2 difficulty and decision design, informed by the river and lake results, plus human pacing review. F26a has added the lake introduction; avoid multiplying similarly short scenarios. Defer goals that require unimplemented local services or per-workplace assignments.
...
- **F26a, implemented: water → fishing dock/boat → food.** Shared fish habitat, shoreline access and complete water/pantry trips distinguish fishing from gardens. Life by the lake introduces the system; F11b2 follows up on difficulty and meaningful choices.
