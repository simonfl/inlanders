...
F23c measured 16 residents with 36 decorations and a 600-frame paused preview. Static model batching reduced draw calls by roughly half in the decorated scene; median frame time improved from 53.2 to 45.3 ms, so performance remains a constraint. F25a can deepen the current village without increasing population. Before larger settlements, revisit resident/cargo rigs and rendering/shadow costs; 24-resident performance is still unverified. The preview showed stable node count, so no speculative cache rewrite is planned.
