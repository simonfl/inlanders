...
**Next hypothesis: one meaningful home-and-land transformation**, selected by [whole-game review75](REVIEW_CHECKPOINT_75.md). The public village is easier to arrange and watch, and the grouped/bank layouts now match productive inventory. But the opening may already solve the interesting problem; a personally wanted intervention remains unproven. Stop automatic interaction and catalogue expansion.

Test a visibly awkward but viable everyday journey and competing use of useful ground. Existing field/home moves, crossings, paths and meal places must support at least two distinct, recoverable responses. Preserve a leave-it option and optional finishing. No prescribed bridge, hunger emergency, population quota or new need. Compare against one current matched opening; reject a dominant answer or a payoff visible only in statistics. Human feedback can replace this hypothesis.
