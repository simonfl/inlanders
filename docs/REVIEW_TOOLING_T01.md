...
These are before/after images, not an animation recording or human playtest. Actual motion and audio still need observation. After the HUD stall fix, a five-second movie retry completed in about 14 recording seconds. Use `-Movie -ObserveSeconds 5 -Speed 1` for a finalized AVI with engine audio at 24 fixed FPS; omission of ObserveSeconds defaults to five. Godot records the 1440x900 project viewport regardless of window size. Movie timing is encoding/recording cost, not native performance. Actual listening and longer-clip validation remain separate.
...
`Review.ps1` separates build, preparation, snapshot checking and inspection. [T04 now separates full build and fixture input fingerprints](REVIEW_CACHE_T04.md). UI-only edits rebuild presentation while reusing unchanged simulation fixtures. Simulation/generator/config/SDK changes invalidate prepared worlds, and snapshot bytes must still match. New captures preserve preparation provenance alongside current build identity; reopening a captured bundle still requires the full current source fingerprint.
