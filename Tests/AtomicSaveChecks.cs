...
static class AtomicSaveChecks
{
    public static void Run()
    {
        ...
        try
        {
            ...
            using(var held=new FileStream(path,FileMode.Open,FileAccess.Read,FileShare.Read))
                try{world.SaveFile(path);}catch(Exception e) when(e is IOException or UnauthorizedAccessException){failed=true;if((string?)e.Data["SavePath"]!=Path.GetFullPath(path) || (string?)e.Data["SaveOperation"]!="replace target" || (int?)e.Data["ReplacementAttempts"]!=11)throw new Exception("Missing exact atomic failure context",e);}
