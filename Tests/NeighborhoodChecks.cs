...
{
    public static void Run(bool foodLand=false)
    {
        void Check(bool ok,string why){if(!ok)throw new Exception(why);}
        var w=foodLand?World.NewFoodLandChallenge():World.NewNeighborhoodExperiment();
        int expected=8+w.NeighborhoodArrivals;
        Check(!foodLand || w.Bushes.Count==1 && w.NeighborhoodReserveTarget==32,"Food/land starting situation is wrong");
        ...
        Check(w.Population==expected && w.Neighborhood!.Arrived && w.People.Skip(8).All(p=>p.SharedWorker),"Committed shared newcomers missing");
        Check(w.Housed<expected,"Test did not exercise unprepared housing");
        ...
        Console.WriteLine($"PASS: one committed arrival, no housing gate, {expected-8} shared newcomers, real crossing journeys, separate progress and exact saved countdown/arrival continuation.");
    }
}
