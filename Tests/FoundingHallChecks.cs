...
static class FoundingHallChecks
{
    ...
    {
        const string dir="artifacts/founding-hall";Directory.CreateDirectory(dir);
        var baseline=Ready();baseline.SaveFile(dir+"/initial.json");
        foreach(string arm in new[]{"home-side","stone-side","awkward"})
        {
            ...
            var hall=Place(w,BuildingKind.GatheringHall,arm=="home-side"?new(-2,3):arm=="stone-side"?new(15,5):new(8,-10));
