...
static class TransformationChecks
{
    ...
    public static void Run()
    {
        ...
        foreach(string arm in new[]{"unchanged","crossing","gardens","mixed","grain","recovered"})
        {
            ...
            int addedCost=w.Cottages.Skip(8).Sum(c=>c.Required);if(arm is "gardens" or "mixed" or "grain")Check(addedCost==12,"Food comparison investment differs");
            ...
            trace.AddRange(w.Cottages.Skip(8).Select(c=>$"Planned {c.Kind} at {c.Cell}, facing {c.Rotation}"));
            ...
            w.SaveFile(folder+"/"+arm+".json");rows.Add(new{arm,hungry,walking,food=w.EdibleStored,buildings=w.Cottages.Count,addedTimber=w.Cottages.Skip(8).Sum(c=>c.Required),trace});
        }
