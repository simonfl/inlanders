...
static class WorkplaceFoodComparison
{
    ...
    {
        const string folder="artifacts/workplace-food-comparison";Directory.CreateDirectory(folder);
        ...
        foreach(bool local in new[]{false,true})foreach(string plan in new[]{"restore-foraging","three-gardens","two-farms-two-bakeries"})
        {
            ...
            if(plan=="restore-foraging")NeighborhoodComparison.PlaceNear(w,BuildingKind.ForagerHut,new(8,2));
            else
            {
            ...
            results.Add(new{name,firstFed,last300AnyHungerSeconds=anyHunger,last300FedFraction=1-hungryPersonSeconds/(300*w.Population),
                ...
                vegetables=w.Food.GrownVegetables,bread=w.Food.BakedBread,activity,samples});
        ...
        File.WriteAllText($"{folder}/manifest.json",JsonSerializer.Serialize(new{status="complete",sources,arms=results.Count,seconds=2400,step=.1,wallSeconds=timer.Elapsed.TotalSeconds},json));
    }
}
