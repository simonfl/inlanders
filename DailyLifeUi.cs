...
public partial class Game
{
    ...
    private void MakeDailyLifeUi()
    {
        ...
        _dailySource=Button("Inspect food",()=>{if(_dailyJourney?.Source is int id){_dailyPerson=-1;SelectBuilding(id);ShowInspector();}else{_focus=OnGround(_world.Stockpile.X,_world.Stockpile.Z);UpdateCamera();}});actions.AddChild(_dailySource);
    ...
    private void RenderDailyLife()
    {
        ...
        bool show=_world.IsArrangementCourt && _dailyPerson>=0 && !_atMainMenu && !_watching && !_placing && !_drawer.Visible && !_inspector.Visible;
