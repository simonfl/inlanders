...
public partial class Game
{
    ...
    private void HamletMenu(bool cultivatedBank,bool groupedFarmsteads=false)
    {
        MenuPage(new HamletProfile(false,cultivatedBank,groupedFarmsteads).Title);
        _mainColumn.AddChild(Text("Twelve neighbors. A working hamlet to make your own.",20,true));
        _mainColumn.AddChild(Text(groupedFarmsteads?"Homes face smaller working clearings beside fields and a kitchen garden. The same people and productive land as the cultivated bank, arranged differently.":cultivatedBank?"Homes share two working fields and a kitchen garden. Four planks can furnish one home. Keep what you like; change what you want.":"A compact hamlet with kitchen gardens. Four planks can furnish one home. Keep what you like; change what you want.",16,true));
        foreach(bool relaxed in new[]{false,true})
        {
            var profile=new HamletProfile(relaxed,cultivatedBank,groupedFarmsteads);
            ...
            MenuButton("New "+label,()=>{if(File.Exists(path))ConfirmMenu("Begin again?","Replace this "+label+"?",Start,()=>HamletMenu(cultivatedBank,groupedFarmsteads));else Start();});
        }
        ...
        MenuButton("About this place",()=>{
            ...
            MenuButton("Back",()=>HamletMenu(cultivatedBank,groupedFarmsteads));
        });
