...
public partial class Game
{
    private async void RunMainMenuSmoke()
    {
        ...
        try
        {
            ...
            _musicRest.Stop();_music.Play(6);float musicPosition=_music.GetPlaybackPosition();
            var musicStream=_music.Stream;ulong musicPlayer=_music.GetInstanceId();
            ...
            Check(_music.Playing && _music.GetPlaybackPosition()>musicPosition,"Menu settings stopped or restarted music");
            await Capture("artifacts/f17-menu-settings.png"); await MenuClick("Back");
            await MenuClick("Earlier prototypes"); await MenuClick("Free play"); await MenuClick("New Original clearing");
            Check(!_atMainMenu && _hud.Visible && _world.Map.OriginalOutline && _paused, "New free play failed");
            Check(_music.Stream==musicStream && _music.GetInstanceId()==musicPlayer && _music.GetPlaybackPosition()>musicPosition,"Entering settlement replaced or restarted music");
            _musicGapIndex=0;_music.Seek(95.8f);
            await ToSignal(GetTree().CreateTimer(.6),SceneTreeTimer.SignalName.Timeout);
            Check(!_music.Playing && _musicRest.TimeLeft>16,"Menu test did not reach quiet interval");
            double quietRemaining=_musicRest.TimeLeft;
            ...
            Check(!_music.Playing && _musicRest.TimeLeft>0 && _musicRest.TimeLeft<quietRemaining,"Menu/Continue restarted or discarded quiet interval");
            ReturnToMainMenu(); await Frames();
            await MenuClick("Earlier prototypes"); await MenuClick("Free play"); await MenuClick("New Three clearings");
            ...
            ReturnToMainMenu(); await Frames(); await MenuClick("Earlier prototypes"); await MenuClick("Campaign"); await MenuClick("Start level 1");
            ...
            ReturnToMainMenu(); await Frames(); await MenuClick("Earlier prototypes"); await MenuClick("Campaign"); await MenuClick("Replay level 1");
            ...
            await MenuClick("Earlier prototypes"); await MenuClick("Free play"); await MenuClick("Resume Original clearing"); Check(_world.SaveJson() == original, "Recovery through separate save failed");
            ReturnToMainMenu(); await Frames(); await MenuClick("Earlier prototypes"); await MenuClick("Free play"); await MenuClick("New Original clearing");
            ReturnToMainMenu(); await Frames(); await MenuClick("Earlier prototypes"); await MenuClick("Free play"); await MenuClick("Restore previous Original clearing");
            ...
            ShowMainMenu(); await Frames(); Check(_mainButtons["Continue"].Disabled, "Continue without a session should be disabled");
            await MenuClick("Earlier prototypes"); await MenuClick("Free play"); await MenuClick("Resume Three clearings");
            ...
            ReturnToMainMenu(); await Frames(); await MenuClick("Earlier prototypes"); await MenuClick("Legacy Creative"); await MenuClick("New Original clearing");
            ...
            ReturnToMainMenu(); await Frames(); await MenuClick("Earlier prototypes"); await MenuClick("Legacy Creative"); await MenuClick("New Original clearing");
            ReturnToMainMenu(); await Frames(); await MenuClick("Earlier prototypes"); await MenuClick("Legacy Creative"); await MenuClick("Restore previous Original clearing");
            ...
            ReturnToMainMenu(); await Frames(); await MenuClick("Earlier prototypes"); await MenuClick("Free play"); await MenuClick("Resume Original clearing");
