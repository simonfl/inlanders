...
public partial class Game
{
    private async void RunHudSmoke()
    {
        try { await CheckHud();
            ...
            GD.Print(OS.GetCmdlineUserArgs().Contains("--civic-identities") ? "SMOKE PASS: civic identities." : OS.GetCmdlineUserArgs().Contains("--catalog-keyboard") ? "SMOKE PASS: catalog keyboard." : OS.GetCmdlineUserArgs().Contains("--decoration-brush") ? "SMOKE PASS: decoration brush." : OS.GetCmdlineUserArgs().Contains("--fences") ? "SMOKE PASS: connected fences." : OS.GetCmdlineUserArgs().Contains("--cottage-finishes") ? "SMOKE PASS: cottage finishes." : OS.GetCmdlineUserArgs().Contains("--river-meadow") ? "SMOKE PASS: river meadow." : OS.GetCmdlineUserArgs().Contains("--finale-campaign") ? "SMOKE PASS: finale campaign." : OS.GetCmdlineUserArgs().Contains("--bread-supply") ? "SMOKE PASS: bread supply investigation." : OS.GetCmdlineUserArgs().Contains("--finale-prototype") ? "SMOKE PASS: finale prototype captures." : OS.GetCmdlineUserArgs().Contains("--shore-art") ? "SMOKE PASS: shoreline art." : OS.GetCmdlineUserArgs().Contains("--woods-art") ? "SMOKE PASS: woodland art." : OS.GetCmdlineUserArgs().Contains("--woods-campaign") ? "SMOKE PASS: woodland campaign." : OS.GetCmdlineUserArgs().Contains("--quarry-campaign") ? "SMOKE PASS: quarry campaign." : OS.GetCmdlineUserArgs().Contains("--goals-only") ? "SMOKE PASS: campaign goals." : OS.GetCmdlineUserArgs().Contains("--comfort-review") ? "SMOKE PASS: focused comfort visual review captures." : OS.GetCmdlineUserArgs().Contains("--camera-drag-only") ? "SMOKE PASS: camera dragging and input isolation." : OS.GetCmdlineUserArgs().Contains("--comfort-only") ? "SMOKE PASS: focused home comfort UI and visuals." : OS.GetCmdlineUserArgs().Contains("--watch-only") ? "SMOKE PASS: focused Watch and clean-view checks." : OS.GetCmdlineUserArgs().Contains("--hud-services-only") ? "SMOKE PASS: focused service/household HUD checks." : "SMOKE PASS: responsive HUD, menu/context controls, all building previews, rotation, rejection feedback, repeat planting, and preview state isolation."); GetTree().Quit(); }
        catch (Exception e) { GD.PrintErr("HUD SMOKE FAIL: " + e); GetTree().Quit(1); }
    }
    private async Task CheckHud()
    {
        if(OS.GetCmdlineUserArgs().Contains("--gateways")){_paused=true;await CheckGatewayUi();return;}
        if(OS.GetCmdlineUserArgs().Contains("--terrain-shaping")){_paused=true;await CheckTerrainShapingUi();return;}
        ...
        if(OS.GetCmdlineUserArgs().Contains("--goals-keyboard")) { _paused=true;await CheckGoalsKeyboard();return; }
        if(OS.GetCmdlineUserArgs().Contains("--economy-keyboard")) { _paused=true;await CheckEconomyKeyboard();return; }
        ...
        if(OS.GetCmdlineUserArgs().Contains("--decoration-brush")) { _paused=true;await CheckDecorationBrush();return; }
        if(OS.GetCmdlineUserArgs().Contains("--fences")) { _paused=true;await CheckFences();return; }
        if(OS.GetCmdlineUserArgs().Contains("--cottage-finishes")) { _paused=true;await CheckCottageFinishes();return; }
        if(OS.GetCmdlineUserArgs().Contains("--river-meadow")) { _paused=true;await CheckRiverMeadow();return; }
        if(OS.GetCmdlineUserArgs().Contains("--finale-campaign")) { _paused=true;await CheckFinaleCampaignUi();return; }
        if(OS.GetCmdlineUserArgs().Contains("--bread-supply")) { _paused=true;await CheckBreadSupplyUi();return; }
        if(OS.GetCmdlineUserArgs().Contains("--finale-prototype")) { _paused=true;await CheckFinalePrototype();return; }
        if(OS.GetCmdlineUserArgs().Contains("--shore-art")) { _paused=true;await CheckShoreArt();return; }
        if(OS.GetCmdlineUserArgs().Contains("--woods-art")) { _paused=true;await CheckWoodsArt();return; }
        if(OS.GetCmdlineUserArgs().Contains("--woods-campaign")) { _paused=true;await CheckWoodsCampaignUi();return; }
        ...
        foreach (var size in new[] { new Vector2I(1440,900), new(1280,720), new(960,640) })
        {
            ...
            await Press(Key.B); Check(_catalogKeyboard && _drawer.Visible, "B did not return to catalog focus");
            await Press(Key.B); Check(!_drawer.Visible, "Focused Build menu did not collapse");
