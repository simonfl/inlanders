...
public partial class Game
{
    private async void RunHudSmoke()
    {
        try { await CheckHud();
            ...
            GD.Print(OS.GetCmdlineUserArgs().Contains("--cottage-finishes") ? "SMOKE PASS: cottage finishes." : OS.GetCmdlineUserArgs().Contains("--river-meadow") ? "SMOKE PASS: river meadow." : OS.GetCmdlineUserArgs().Contains("--finale-campaign") ? "SMOKE PASS: finale campaign." : OS.GetCmdlineUserArgs().Contains("--bread-supply") ? "SMOKE PASS: bread supply investigation." : OS.GetCmdlineUserArgs().Contains("--finale-prototype") ? "SMOKE PASS: finale prototype captures." : OS.GetCmdlineUserArgs().Contains("--shore-art") ? "SMOKE PASS: shoreline art." : OS.GetCmdlineUserArgs().Contains("--woods-art") ? "SMOKE PASS: woodland art." : OS.GetCmdlineUserArgs().Contains("--woods-campaign") ? "SMOKE PASS: woodland campaign." : OS.GetCmdlineUserArgs().Contains("--quarry-campaign") ? "SMOKE PASS: quarry campaign." : OS.GetCmdlineUserArgs().Contains("--goals-only") ? "SMOKE PASS: campaign goals." : OS.GetCmdlineUserArgs().Contains("--comfort-review") ? "SMOKE PASS: focused comfort visual review captures." : OS.GetCmdlineUserArgs().Contains("--camera-drag-only") ? "SMOKE PASS: camera dragging and input isolation." : OS.GetCmdlineUserArgs().Contains("--comfort-only") ? "SMOKE PASS: focused home comfort UI and visuals." : OS.GetCmdlineUserArgs().Contains("--watch-only") ? "SMOKE PASS: focused Watch and clean-view checks." : OS.GetCmdlineUserArgs().Contains("--hud-services-only") ? "SMOKE PASS: focused service/household HUD checks." : "SMOKE PASS: responsive HUD, menu/context controls, all building previews, rotation, rejection feedback, repeat planting, and preview state isolation."); GetTree().Quit(); }
