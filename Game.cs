...
public partial class Game : Node3D
{
    ...
    private void Reset()
    {
        ...
        try
        {
            var next = _world.PublicPlace is {} place?place.Create():_world.Founding?.WorkingVillage==true?World.NewWorkingVillage():_world.Founding?.RiverFarmstead==true?World.NewRiverFarmstead():_world.Founding!=null?World.NewFoundingSettlement():_world.CourtStudy is {} study?World.NewCourtExperience(study.Finite):_world.IsArrangementCourt ? (_world.Creative?World.NewCreativeCourt():World.NewArrangementCourt()) : _world.IsInheritedShoreline ? World.NewInheritedShoreline() : _world.Neighborhood?.FoodLandChallenge==true ? World.NewFoodLandChallenge() : _world.HasWorkplaceFood ? World.NewWorkplaceFoodExperiment() : _world.Neighborhood!=null ? (_world.Map.Name=="Landing and meadow — experiment" ? World.NewNeighborhoodLandscapeExperiment() : World.NewNeighborhoodExperiment()) : _world.Creative ? World.NewCreative(_world.Map.Name == "Three clearings") : _world.Map.Name == "Three clearings" ? World.NewLargeMap() : World.NewScenario();
    ...
    {
        UpdateStorybookSpaces();UpdateWorkedLandscape();
