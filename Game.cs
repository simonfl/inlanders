...
public partial class Game : Node3D
{
    ...
    public override void _Ready()
    {
        ...
        if (OS.GetCmdlineUserArgs().Contains("--pantry-smoke-test")) CallDeferred(MethodName.RunPantrySmoke);
    ...
    private void Reset()
    {
        ...
        try
        {
            var next = _world.Founding?.TransformationHamlet==true?World.NewTransformationHamlet(_world.Creative):_world.Founding?.WorkingVillage==true?World.NewWorkingVillage():_world.Founding?.RiverFarmstead==true?World.NewRiverFarmstead():_world.Founding!=null?World.NewFoundingSettlement():_world.CourtStudy is {} study?World.NewCourtExperience(study.Finite):_world.IsArrangementCourt ? (_world.Creative?World.NewCreativeCourt():World.NewArrangementCourt()) : _world.IsInheritedShoreline ? World.NewInheritedShoreline() : _world.Neighborhood?.FoodLandChallenge==true ? World.NewFoodLandChallenge() : _world.HasWorkplaceFood ? World.NewWorkplaceFoodExperiment() : _world.Neighborhood!=null ? (_world.Map.Name=="Landing and meadow — experiment" ? World.NewNeighborhoodLandscapeExperiment() : World.NewNeighborhoodExperiment()) : _world.Creative ? World.NewCreative(_world.Map.Name == "Three clearings") : _world.Map.Name == "Three clearings" ? World.NewLargeMap() : World.NewScenario();
    ...
    public override void _Process(double delta)
    {
        ...
        if (!EditingText && !_catalogKeyboard && !_peopleKeyboard && !_economyKeyboard && !_goalsKeyboard && !_surveyKeyboard && pan != Vector3.Zero) { _watchOrbit=false; _followPerson = false; _focus += pan.Rotated(Vector3.Up, _angle) * dt * Math.Max(7, _camera.Size * 0.35f); UpdateCamera(); }
    ...
    private void RenderActors(float dt)
    {
        ...
        foreach (var h in _world.Cottages)
        {
            ...
            int viewKey = h.Kind == BuildingKind.Stockpile ? stage * 100 + h.StoredLogs + h.StoredPlanks + h.StoredStone + (h.StorageMaterial==Resource.Stone?2000:h.StorageMaterial==Resource.Planks?1000:0) : stage;
