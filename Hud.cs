...
public partial class Game
{
    ...
    private void MakeUi()
    {
        ...
        MakeAreaRemovalUi();MakeBushMoveUi();MakeGatheringPlanUi();MakeTerrainUi();
    ...
    private void MakeBuildMenu(VBoxContainer column)
    {
        ...
        _connectPathsButton = Button("Connect paths · two clicks", () => TogglePaths(3)); column.AddChild(_connectPathsButton);
    ...
    private void MakeGoalsMenu(VBoxContainer column)
    {
        ...
        MakeNeighborhoodGoals(column);MakeCourtExperienceUi(column);MakeFoundingUi(column);
    ...
    private void UpdateHud()
    {
        ...
        _foodStatus.Text = _world.PublicPlace is {Relaxed:true}?"Relaxed":_world.Creative ? (_world.IsArrangementCourt?"Free arrangement":"Creative") : _world.Food.Hunger > 0 ? "Hungry" : "Well fed";
        ...
        UpdateAreaRemoval();UpdateBushMove();UpdateGatheringPlan();UpdateTerrainUi();
        _hint.Text = _placing ? (_woodlandTool>0 ? $"{WoodlandToolName} · click or drag · Esc finishes" : _decorating ? (_removeDecoration ? "Remove decorations · click · Esc finishes" : (_decorationKind==DecorationKind.Gateway?"Fence gateway · always open · paths pass through · R rotates · Esc finishes":$"{DecorationName(_decorationKind)} · free · R rotates · Esc finishes")) : _pathTool > 0 ? (_pathTool == 1 ? "Paint paths · drag or click · Esc finishes" : "Remove paths · drag or click · Esc finishes") : _clearingTrees ? (_world.Creative ? "Clear immediately · recover timber · Esc finishes" : "Clear trees & stumps · click to mark/cancel · Esc finishes") : _plantingTrees ? "Plant alders · click to mark · Esc finishes" : $"{BuildingName(_buildKind)} · {BuildCost(_buildKind)} · {(_buildKind == BuildingKind.Bridge ? "1 water tile" : _buildKind == BuildingKind.FishingDock ? "1 shore tile + launch" : _buildKind == BuildingKind.SeatingGarden ? "1 tile" : _rotation%2!=0 ? $"{Buildings.Get(_buildKind).Depth} × {Buildings.Get(_buildKind).Width}" : $"{Buildings.Get(_buildKind).Width} × {Buildings.Get(_buildKind).Depth}")} · R / Shift+R rotates · Esc cancels") : "";
