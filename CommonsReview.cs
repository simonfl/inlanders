...
public partial class Game
{
    private async Task ProbeCommons()
    {
        ...
        var center=_world.Map.Land.Where(c=>_world.CommonsProblem(c)==null).OrderBy(c=>(c.Point-(_world.Founding!=null?new Cell(-1,5):new Cell(17,9)).Point).LengthSquared()).First();
        ...
        await OpenMenu(2);await UiClick(_world.Founding!=null?_foundingCommons:_commonsEntry);await Frames();await Press(Key.Escape);Check(_world.SaveJson()==saved,"Cancelled rearrangement changed place");
        await OpenMenu(2);await UiClick(_world.Founding!=null?_foundingCommonsRemove:_commonsRemove);await Frames();Check(_world.Commons==null,"Commons removal failed");
